...
def enc(cfg):
	...
	if cfg.modality == 'pixels':
		...
		layers = [NormalizeImg(),
				  nn.Conv2d(C, cfg.num_channels, 1, stride=1), nn.ReLU(),
				  nn.Conv2d(cfg.num_channels, cfg.num_channels, 1, stride=1), nn.ReLU(),
				  nn.Conv2d(cfg.num_channels, cfg.num_channels, 1, stride=1), nn.ReLU(),
				  nn.Conv2d(cfg.num_channels, cfg.num_channels, 1, stride=1), nn.ReLU()]
...
class Episode(object):
	...
	def add(self, obs, action, reward, done):
		...
		self.reward[self._idx] = torch.ones(1)*reward
...
class ReplayBuffer():
	"""
	Storage and sampling functionality for training TD_MPC / TOLD.
	...
	def __init__(self, cfg):
		...
		self._obs = torch.empty((int(self.capacity+1), *obs_shape), dtype=dtype, device=self.device)
		self._last_obs = torch.empty((int(self.capacity//cfg.episode_length), *cfg.obs_shape), dtype=dtype, device=self.device)
		self._action = torch.empty((int(self.capacity), cfg.action_dim), dtype=torch.float32, device=self.device)
		self._reward = torch.empty((int(self.capacity),), dtype=torch.float32, device=self.device)
		self._priorities = torch.ones((int(self.capacity),), dtype=torch.float32, device=self.device)
		self._eps = 1e-6
		self._full = False
		self.idx = 0
