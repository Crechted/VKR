...
class TOLD(nn.Module):
	"""Task-Oriented Latent Dynamics (TOLD) model used in TD_MPC."""
	def __init__(self, cfg):
		super().__init__()
		self.cfg = cfg
		self._encoder = h.enc(cfg)
		self._encoder.to(device=torch.device(cfg.device))
...
class TDMPC():
	"""Implementation of TD_MPC learning + inference."""
	def __init__(self, cfg):
		self.cfg = cfg
	...
	def plan(self, obs, eval_mode=False, step=None, t0=True):
		"""
		Plan next action using TD_MPC inference.
